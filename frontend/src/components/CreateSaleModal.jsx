...
export default function CreateSaleModal({ isOpen, onClose, refreshSales }) {
  ...
  // Synchronize full payments automatically when order values shift
  useEffect(() => {
    if (!isPartialPayment) {
      setAmountPaid(total > 0 ? total.toFixed(2) : '');
    }
  }, [total, isPartialPayment]);
  ...
  const submit = async () => {
    ...
    // Explicit Balance Validation Check
    const parsedAmountPaid = parseFloat(amountPaid) || 0;
    if (parsedAmountPaid < 0) {
      showNotification(
        'Payment entry rejection: Tendered capital cannot register as a negative.',
        'warning'
      );
      return;
    }
    ...
    if (parsedAmountPaid > total) {
      showNotification(
        `Overpayment alert: Customer cannot settle amounts over the calculated total of R ${total.toFixed(2)}`,
        'warning'
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        payments: [
          {
            amount: parseFloat(parsedAmountPaid.toFixed(2)),
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 transition-all duration-300">
      <div className="relative w-full max-w-5xl bg-[#0f172a] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-8 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
          ...
          {/* FINANCIAL SETTLEMENT ACCOUNTING PANEL */}
          <div className="bg-[#1e293b]/20 border border-slate-800/80 rounded-xl p-6 space-y-5">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-2">
              <Coins size={16} /> Financial Settlement & Terms
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 bg-[#0b0f19] p-1.5 border border-slate-800 rounded-xl">
                  <button
                    type="button"
                    disabled={isSubmitting}
              ...
              <div className="space-y-2">
                <label className="text-sm text-slate-300 font-medium">
                  Amount Tendered / Paid (R)
                </label>
                <input
                  type="number"
                  step="0.01"
                  disabled={isSubmitting || !isPartialPayment}
                  value={amountPaid}
                  onChange={(e) => setAmountPaid(e.target.value)}
                  placeholder="0.00"
                  className="w-full bg-[#0b0f19] border border-slate-800 focus:border-emerald-500/50 rounded-xl px-4 py-3 text-base font-bold text-white placeholder-slate-600 outline-none transition-all disabled:opacity-60 disabled:bg-[#111625]"
                />
              </div>

              <div className="space-y-2">
                <label className="text-sm text-slate-300 font-medium">
                  Transaction Audit Reference
                </label>
                <input
                  disabled={isSubmitting}
                  value={reference}
                  onChange={(e) => setReference(e.target.value)}
                  placeholder="e.g. POP ref # or Notes"
                  className="w-full bg-[#0b0f19] border border-slate-800 focus:border-emerald-500/50 rounded-xl px-4 py-3 text-base text-white placeholder-slate-600 outline-none transition-all"
                />
              </div>
            </div>
          </div>
        ...
        <div className="px-8 py-6 border-t border-slate-800 bg-[#1e293b]/40 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <div className="bg-[#0b0f19] border border-slate-800 rounded-xl px-6 py-3 min-w-[160px] text-center sm:text-left">
            ...
            {isPartialPayment && (
              <div className="bg-[#0b0f19] border border-slate-800 rounded-xl px-6 py-3 min-w-[160px] text-center sm:text-left animate-in fade-in zoom-in-95 duration-200">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 block">
                  Balance Due
                </span>
